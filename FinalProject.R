...
par(mfrow=c(2,3))
barplot(denied_perc, main="Verdict of Applicants Loan Approval",ylab = "Percent",
...
barplot(DeniedPMI_perc,main="Percent of Applicants denied PMI", ylab = "Percent",
...
DF_D1 <- data.frame(data$denied,I(data$HouseExp_Income*data$netWealth),
                    data$cCredHist, data$mCredHist,data$pubRec,data$Unemply,
...
DF_D2 <- data.frame(data$denied, I(data$HouseExp_Income+data$Education_yrs), 
                    ...
                    log(data$Unemply), data$SelfEmp, data$DeniedPMi, data$nonWhite, 
...
rm(HouseExp_Income,TotalDebtPmt_Income,netWealth,cCredHist,
   ...
   loanAppraised_Medium,DeniedPMi,nonWhite,NotMarried,Education_yrs,denied)
# data frames
DF_D3 <- data.frame(data$denied, data$HouseExp_Income, data$TotalDebtPmt_Income,
                    data$netWealth, data$cCredHist, data$mCredHist, data$Unemply,
                    data$SelfEmp, data$loanAppraised_Low , data$loanAppraised_Medium,
...
for (i in 1:4){
    for (j in 1:4){
        for (k in 1:4){
            for (l in 1:4){
                model_d3 <- glm(denied~ HouseExp_Income + poly(TotalDebtPmt_Income,i,raw=TRUE)
                                + netWealth + poly(cCredHist,j,raw=TRUE) + mCredHist
                                + poly(Unemply,k,raw=TRUE) + SelfEmp + loanAppraised_Low 
                                + loanAppraised_Medium + loanAppraised_High 
                                + DeniedPMi  + nonWhite + NotMarried 
                                + poly(Education_yrs,l,raw=TRUE),
...
DF_D3a <- data.frame(data$denied, data$HouseExp_Income, I(data$TotalDebtPmt_Income**4),
                     data$netWealth, I(data$cCredHist**3), data$mCredHist,
                     I(data$Unemply**4), data$SelfEmp, data$loanAppraised_Low, 
                     data$loanAppraised_Medium, data$nonWhite, data$loanAppraised_High, 
                     data$DeniedPMi,data$nonWhite, data$NotMarried, I(data$Education_yrs**4))
...
#data from D2
bestData <- data.frame(data$denied, I(data$HouseExp_Income+data$Education_yrs), 
                       data$cCredHist, data$mCredHist, data$pubRec, log(data$Unemply), 
                       data$SelfEmp, data$DeniedPMi, data$nonWhite, data$NotMarried, 
